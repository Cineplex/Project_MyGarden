...
struct EditReminderView: View {
    ...
    init(reminderViewModel: ReminderViewModel, plantViewModel: PlantViewModel, reminder: Reminder, userPlants: [Plant]) {
        self.reminderViewModel = reminderViewModel
        self.plantViewModel = plantViewModel
        self.reminder = reminder
        self.userPlants = userPlants
        
        // ตั้งค่า initial values จาก reminder ที่มีอยู่
        _reminderTitle = State(initialValue: reminder.title ?? "")
        _selectedPlantId = State(initialValue: reminder.plantId)
        _selectedTime = State(initialValue: reminder.time)
        _selectedDays = State(initialValue: reminder.daysOfWeek)
    ...
    var body: some View {
        NavigationView {
            ...
            .alert("ยืนยันการลบ", isPresented: $showDeleteAlert) {
                ...
                Text("คุณแน่ใจหรือไม่ว่าต้องการลบการแจ้งเตือนนี้?")
            }
        }
    }
    ...
    private func saveReminder() {
        ...
        updatedReminder.title = title.isEmpty ? nil : title
        updatedReminder.plantId = plantId
        updatedReminder.time = selectedTime
        updatedReminder.daysOfWeek = selectedDays
